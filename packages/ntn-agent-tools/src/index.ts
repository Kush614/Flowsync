...
const code = (s: string, language: string) => ({
	object: "block",
	type: "code",
	code: { rich_text: [{ type: "text", text: { content: s } }], language },
});
...
worker.tool("generate_changelog", {
	...
	execute: async (input) => {
		...
		const pie =
			`pie showData\n  title ${tag} — change breakdown\n` +
			cats.map(([k, n]) => `  "${k}" : ${n}`).join("\n");
		const flow =
			`flowchart LR\n` +
			`  G["Commits\\n${base}..${tag}"] --> P["FlowSync\\nparse + bucket"]\n` +
			`  P --> N["Notion\\nchangelog page"]\n` +
			`  N -->|"Status: Approved"| R["GitHub\\nrelease"]\n` +
			`  classDef n fill:#1f3a5f,stroke:#7ab8ff,color:#e8e8ea;\n` +
			`  classDef g fill:#1f3a2a,stroke:#7ee0a2,color:#e8e8ea;\n` +
			`  class N n; class R g;`;

		const body: unknown[] = [
			callout(
				`${tag} · range ${base}..${tag} · ${cmp.commits.length} commits · ${today} · auto-generated by FlowSync`,
				"🚀"
			),
			h2("📊 Change breakdown"),
			code(pie, "mermaid"),
		...
		if (breaking.length) body.push(section("⚠️ Breaking changes", breaking));
		...
		body.push(divider());
		body.push(callout("Generated by FlowSync — code is the source of truth, Notion is the control surface.", "🔁"));
